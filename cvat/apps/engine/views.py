    ...
    def about(request):
        ...
        about = {
            ...
            "description": "CVAT is completely re-designed and re-implemented " +
                " shiv version of Video Annotation Tool from Irvine, California " +
...
@api_view(['GET', 'POST', ])
def viewcatlog(request):
    try:
        operations_res = json.loads(serializers.serialize("json", models.Sign.objects.all()))
        print(operations_res)


        res={"status": "success", "message": "get data successfully","data":"sai"}
...
class CatlogViewSet(viewsets.ModelViewSet):
    queryset = Catfolder.objects.all()
    # serializer_class = SignSerializer
    def retrieve(self, request, pk):
        projectid=pk
        print("sai")
        serializer_class = CatfolderSerializer(self.queryset,many=True)
        print("reddy")
    ...
    def data(self,request,pk):
        ...
        queryset = Catlog.objects.filter(projectid=pk)
        print(queryset)
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    #     obj.catalogue_files = self.request.FILES.get('catalogue_files')
    #     obj.labelschema_files = self.request.FILES.get('labelschema_files')
                        ...
                        for val in list_name:
                            ...
                            if list_filenames[1] != "UNASSIGNED":
                                ...
                                if (val[-6:]) == "/image":
                                    ...
                                    if not os.path.exists(filepath):
                                        os.mkdir(filepath)
                                    cv2.imwrite(os.path.join(filepath, list_filenames[2] + ".jpg"), imS)
